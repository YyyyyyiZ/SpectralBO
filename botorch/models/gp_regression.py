...
from gpytorch.kernels import MaternKernel, RBFKernel, RQKernel, ScaleKernel, SpectralMixtureKernel, PeriodicKernel
from botorch.models.kernels import CauchyMixtureKernel
...
class SingleTaskGP(BatchedMultiOutputGPyTorchModel, ExactGP, FantasizeMixin):
    ...
    def __init__(
        ...
        if covar_module == "rbf":
            ...
            covar = MaternKernel(nu=2.5)
        elif covar_module == 'rq':
            covar = RQKernel()
        elif covar_module == 'pe':
            covar = PeriodicKernel()
        elif covar_module == 'gsm':
            covar = SpectralMixtureKernel(num_mixtures=n_mixture, ard_num_dims=transformed_X.shape[-1])
        elif covar_module == 'csm':
            covar = CauchyMixtureKernel(num_mixtures=n_mixture, ard_num_dims=transformed_X.shape[-1])
        else:
